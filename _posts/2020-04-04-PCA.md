...
It's unimportant to find the exact line X', instead compute the direction. Let's assume X' as u<sub>1</sub> [(unit vecor)](https://en.wikipedia.org/wiki/Unit_vector). Let D={x}<sub>i=1</sub><sup>n</sup> be the initial dataset and D'={x'<sub>i</sub>}<sup>n</sup> be the transformed dataset where {x'<sub>i</sub>} are the projected points on u<sub>1</sub>.
